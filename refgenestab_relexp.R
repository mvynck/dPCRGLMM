...
calc.stab.relexp <- function(glmm.df){
	...
	for(i in 1:n){
		for(j in 1:n){
			...
			fit <-glmer(as.matrix(glmm.df.subset[,1:2])~1+					#intercept
					dummy(glmm.df.subset$gene,unique(glmm.df$gene)[i])+		#relative expression
					(1|glmm.df.subset$well)+								#replicate effect
					(1|glmm.df.subset$sample),								#sample effect
					family=binomial(cloglog), nAGQ=1, verbose=F)

...
#for(subject in 1:length(unique(subjects))){
	...
	#references
	AMELX <- cbind(c(rep("AMELX",3)),data[,1],data[,2],c(rep(FALSE,3)))
	ATP11C <- cbind(c(rep("ATP11C",3)),data[,11],data[,12],c(rep(FALSE,3)))
	IL13RA1 <- cbind(c(rep("IL13RA1",3)),data[,5],data[,6],c(rep(FALSE,3)))
#	AMELY <- cbind(c(rep("AMELY",3)),data[,17],data[,18],c(rep(FALSE,3)))
#	SRY <- cbind(c(rep("SRY",3)),data[,23],data[,24],c(rep(FALSE,3)))
	...
	#dataframe
	datamat<-data.frame(rbind(CLIC6,DSCR3,SYNJ1,AMELX,ATP11C,IL13RA1,C18ORF62,ITGBL1,LAMA3,LNX2,SMCHD1,RPP30))
	...
	for(i in 1:length(ref.genes)){
		use.ref <- order(abs(stability[[2]][,3]))[1:i]
	...
	barplot(integer,xlab="Number of reference genes",ylab="Number of integer copy numbers",names.arg=1:9)
	dev.off()
